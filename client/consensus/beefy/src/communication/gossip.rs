...
impl<B: Block> GossipMessage<B> {
	...
	pub fn unwrap_resharing(self) -> Option<u8> {
		match self {
			GossipMessage::Reshare(value) => Some(value),
...
/// Gossip engine resharing messages topic
pub(crate) fn resharing_topic<B: Block>() -> B::Hash
where
	B: Block,
{
	<<B::Header as Header>::Hashing as Hash>::hash(b"etf-resharing")
}
...
impl<B: Block> Filter<B> {
	...
	fn consider_resharing(&self, round: NumberFor<B>, set_id: ValidatorSetId) -> Consider {
		self.inner
			.as_ref()
			.map(|f|
	...
	B: Block,
{
	pub(crate) fn new(
		...
		let val = GossipValidator {
			resharing_topic: resharing_topic::<B>(),
	...
	fn validate_resharing(
		...
		// 	match filter.consider_commitment(round, set_id) {
		...
		// // do I need to do this still? maybe I just keep all of these for now...
		// if BeefyKeystore::verify(&vote.id, &vote.signature, &vote.commitment.encode()) {
		// 	Action::Keep(self.commitment_topic, benefit::VOTE_MESSAGE)
		// } else {
		// 	debug!(
		// 		target: LOG_TARGET,
		...
		// }
		Action::Keep(self.resharing_topic, benefit::RESHARING)
	}
	...
	B: Block,
{
	...
	fn validate(
		...
		let action = match GossipMessage::<B>::decode_all(&mut data) {
			Ok(GossipMessage::Reshare(value)) => self.validate_resharing(value, sender),
	...
	fn message_expired<'a>(&'a self) -> Box<dyn FnMut(B::Hash, &[u8]) -> bool + 'a> {
		...
		Box::new(move |_topic, mut data| match GossipMessage::<B>::decode_all(&mut data) {
			Ok(GossipMessage::Reshare(value)) => {
				// TODO
				false
			},
