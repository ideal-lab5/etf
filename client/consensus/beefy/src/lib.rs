...
use crate::communication::gossip::resharing_topic;
	...
	R::Api: BeefyApi<B, AuthorityId>,
{
	...
	pub async fn async_initialize(
		...
		// fetch shares from runtime here
		// then run ACSS Recover and submit


		// this is a test... and also doesn't work...
		// maybe it would make more sense to gossip the messages instead?
		// then encode in rt storage when recieved
		// runtime.runtime_api()
		...
		// TODO
		// let initial_shares: Option<Vec<u8>> = 
		// 	runtime.runtime_api().read_share(best_grandpa.hash(), 0).unwrap();
		// debug!(
		// 	target: LOG_TARGET,
		// 	"🎲 Found initial shares for ETF: {:?}.",
		// 	initial_shares
		// );
		// key_store.
		// if let Ok(r, r_prime) = HighThresholdACSS::recover(sk, initial_shares) {
		// 	// omg it worked

		// }

		// run (s, s') <- ACSS.Recover(my_shares) 
		// c = sG + s'H, then submit extrinsic to encode in RT storage
		// runtime_api
		// 	.submit_report_equivocation_unsigned_extrinsic(best_block_hash, proof, key_owner_proof)
		// 	.map_err(Error::RuntimeApi)?;

		// we need to wait for each ETF authority to run the ACSS recover algorithm
		// and also submit their shares
		// let _ready = wait_for_zellijcommitments().await?;
	...
	async fn init_state(
		...
		// let initial_shares: Option<Vec<u8>> = 
		// 	runtime.runtime_api().read_share(best_grandpa.clone().hash(), 0).unwrap();
		...
		// self.comms.gossip_engine.gossip_message(resharing_topic::<B>(), 1u8, false);
		// then run acss recovery and gossip the commitments?
		// and add the recovered secrets to the aux db for now
	...
	loop {
		...
		let worker_builder = loop {
			futures::select! {
				...
				// _ = &mut beefy_comms.gossip_engine.gossip_message(resharing_topic::<B>(), vec![1u8], false) => {
				// 	continue
				// },
			}
		};
		...
		let worker = worker_builder.build(
			...
			BTreeMap::new(),
				...
				// can we do it now or do we need to do it in the worker?
		// beefy_comms.gossip_engine.gossip_message(resharing_topic::<B>(), vec![1u8], false);
