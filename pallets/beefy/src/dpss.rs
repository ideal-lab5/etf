...
pub struct ResharingMetadata<Offender, N>
where
...
impl<Offender: Clone, N> Offence<Offender> for ResharingMetadata<Offender, N>
where
	N: Copy + Clone + PartialOrd + Ord + Eq + PartialEq + Encode + Decode,
{
	const ID: Kind = *b"etf:acssprotocol";
	...
	// in our case I suppose this would be effectively used when
    // a validator proposed bad shares or a bad commitment?
    // for now let's assume we only need to perform ACSS recovery
	fn slash_fraction(&self, offenders_count: u32) -> Perbill {
        ...
        Perbill::from_rational(0u32, 0u32).square()
	}
}
...
pub struct CommitmentReportSystem<T, R, P, L>(sp_std::marker::PhantomData<(T, R, P, L)>);

/// Equivocation evidence convenience alias.
pub type CommitmentFor = u8;

impl<T, R, P, L> OffenceReportSystem<Option<T::AccountId>, CommitmentFor>
	for CommitmentReportSystem<T, R, P, L>
where
	T: Config + pallet_authorship::Config + frame_system::offchain::SendTransactionTypes<Call<T>>,
	R: ReportOffence<
		T::AccountId,
		P::IdentificationTuple,
		ResharingMetadata<P::IdentificationTuple, BlockNumberFor<T>>,
	>,
	...
	L: Get<u64>,
{
	...
	fn publish_evidence(evidence: CommitmentFor) -> Result<(), ()> {
		use frame_system::offchain::SubmitTransaction;

		let call = Call::report_commitment_unsigned {
			value: evidence,
		};
		...
		match res {
			...
			Err(e) => error!(target: LOG_TARGET, "Error submitting commitment: {:?}", e),
		}
		res
	}

	fn check_evidence(
		evidence: CommitmentFor,
