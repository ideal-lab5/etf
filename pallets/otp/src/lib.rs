	...
	impl<T: Config> Pallet<T> {
		...
		pub fn proxy(
			...
			if let Some(proxy_details) = Registry::<T>::get(name) {
				...
				let leaves: Vec<Leaf> =
					proof.clone().into_iter().map(|p| Leaf::from(p)).collect::<Vec<_>>();
				let merkle_proof =
					MerkleProof::<Leaf, MergeLeaves>::new(proof.len() as u64, leaves);
				let root = Leaf::from(proxy_details.root);

				ensure!(
					merkle_proof
						.verify(root, vec![(position, Leaf::from(ciphertext_bytes.clone()))])
						.unwrap(),
