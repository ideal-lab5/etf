//! An opt-in utility module for reporting beacon pulses.
//!
...
pub struct PulseReport<Offender, N>
where
...
impl<Offender: Clone, N> Offence<Offender> for PulseReport<Offender, N>
where
	N: Copy + Clone + PartialOrd + Ord + Eq + PartialEq + Encode + Decode,
{
	const ID: Kind = *b"idn:randombeacon";
	...
	fn slash_fraction(&self, offenders_count: u32) -> Perbill {
		...
		Perbill::from_rational(0u32, 0u32)
	}
}

pub type Pulse<BN> = (Vec<Vec<u8>>, BN);
...
pub struct BeaconReportSystem<T, R, P, L>(sp_std::marker::PhantomData<(T, R, P, L)>);

/// Equivocation evidence convenience alias.
pub type PulseEvidenceFor<T> = (
	// Vec<Vec<u8>>,	
	// BlockNumberFor<T>,
...
impl<T, R, P, L> OffenceReportSystem<Option<T::AccountId>, PulseEvidenceFor<T>>
	for BeaconReportSystem<T, R, P, L>
where
	T: Config + pallet_authorship::Config + frame_system::offchain::SendTransactionTypes<Call<T>>,
	R: ReportOffence<
		T::AccountId,
		P::IdentificationTuple,
		PulseReport<P::IdentificationTuple, BlockNumberFor<T>>,
	>,
	...
	L: Get<u64>,
{
	...
	fn publish_evidence(evidence: PulseEvidenceFor<T>) -> Result<(), ()> {
		use frame_system::offchain::SubmitTransaction;
		let (pulse, key_owner_proof) = evidence;

		let call = Call::write_pulse {
			signatures: pulse.0,
			block_number: pulse.1,
			// key_owner_proof
		};
...
// 			return ValidTransaction::with_tag_prefix("BeefyDoStuff")
